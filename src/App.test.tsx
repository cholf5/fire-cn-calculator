...
test("切换轻奢预设后会即时更新主结果", async () => {
  ...
  expect(screen.getByText("推荐准备资产")).not.toBeNull();
  expect(screen.getAllByText("¥ 5,126,214").length).toBeGreaterThan(0);

  await user.click(screen.getByRole("button", { name: "轻奢" }));

  expect(screen.getAllByText("¥ 14,511,845").length).toBeGreaterThan(0);
...
test("URL 参数会覆盖 localStorage 并反映到首屏结果", () => {
  ...
  expect(screen.getAllByText("¥ 10,848,932").length).toBeGreaterThan(0);
});
...
test("点击重置默认值后会恢复默认结果并清空 URL 参数", async () => {
  ...
  expect(screen.getAllByText("¥ 11,005,515").length).toBeGreaterThan(0);
...
test("关键输入字段会通过信息按钮展示参数说明", async () => {
  const user = userEvent.setup();

  render(<App />);

  await user.click(screen.getByRole("button", { name: "查看月支出（不含住房）说明" }));
  expect(
    screen.getByText("这里填写不含房租或房贷的基础月支出，住房部分会按是否有房和城市等级单独估算。"),
  ).not.toBeNull();

  await user.click(screen.getByRole("button", { name: "查看城市等级说明" }));
  expect(
    screen.getByText("只在无房时影响住房补充比例，一线、二线、三四线会采用不同住房压力口径。"),
  ).not.toBeNull();

  await user.click(screen.getByRole("button", { name: "▼ 高级设置" }));

  await user.click(screen.getByRole("button", { name: "查看医疗支出占比说明" }));
  expect(screen.getByText("会按比例抬高总年支出，用于预留基础医疗和健康支出空间。")).not.toBeNull();

  await user.click(screen.getByRole("button", { name: "查看退休年限说明" }));
  expect(screen.getByText("用于估算需要覆盖多少年的退休生活，年限越长，长期覆盖建议通常越高。")).not.toBeNull();
});

test("结果区会展示长期覆盖建议与基础估算说明", () => {
  window.history.replaceState({}, "", "/?m=12000&h=0&c=tier1&med=0.12&ry=40&rg=0.05");

  render(<App />);

  expect(screen.getByText("长期结论")).not.toBeNull();
  expect(screen.getAllByText("长期覆盖建议").length).toBeGreaterThan(0);
  expect(screen.getByText("基于退休年限与未来支出增长估算")).not.toBeNull();
  expect(screen.getByText("系统会优先采用更稳妥的口径，避免把两个结果同时当成最终答案。")).not.toBeNull();
});
...
test("点击复制当前结果摘要后会复制包含主结果和长期参考的纯文本", async () => {
  ...
  expect(copiedText).toContain("推荐准备资产");
  expect(copiedText).toContain("基础估算");
  expect(copiedText).toContain("长期覆盖建议");
...
test("点击复制分享卡片后会生成图片并写入剪贴板", async () => {
  const write = vi.fn().mockResolvedValue(undefined);
  const clipboardItemMock = vi.fn(function ClipboardItemMock(this: object, items: unknown) {
  ...
  vi.spyOn(HTMLCanvasElement.prototype, "toBlob").mockImplementation(function toBlob(callback) {
    callback(new Blob(["card"], { type: "image/png" }));
  });
  ...
  await act(async () => {
    fireEvent.click(screen.getByRole("button", { name: "复制分享卡片" }));
    await Promise.resolve();
  });
...
test("结果区会展示基础估算与长期覆盖建议的对比条", () => {
  window.history.replaceState({}, "", "/?m=12000&h=0&c=tier1&med=0.12&ry=40&rg=0.05");

  render(<App />);

  expect(screen.getByText("目标对比")).not.toBeNull();
  expect(screen.getAllByText("基础估算").length).toBeGreaterThan(0);
  expect(screen.getAllByText("长期覆盖建议").length).toBeGreaterThan(0);
});
